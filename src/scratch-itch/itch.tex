...
%\subsection{The execution log}\label{subsec:the-execution-log}
%
%TODO
...
For example, the action to click a sprite is saved in the log.
Now, in the after execution phase, the event in the log provides access to the snapshot from just before the sprite was clicked and just after the sprite was clicked (or when scripts have terminated if synchronous).
This allows for a test to check if the costumes changes when a sprite is clicked, regardless of the implementation.
This is illustrated in \cref{fig:itch-hat-costum-change}.
