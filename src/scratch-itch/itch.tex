...
Block-based languages like Scratch are popular tools for introducing computer science concepts to young learners~\autocite{bauLearnableProgrammingBlocks2017}.
...
It provides facilities to make common scenarios easy to test, allowing the educators to focus on testing the interesting parts of an exercise.
...
In this exercise, there are three sprites: each sprite must grow if the \keys{g} key is pressed.
Additionally, the sprites must shrink if the \keys{s} key is pressed.
...
These require the project to be executed.
The during and after execution phases are needed for dynamic tests.
...
Testing can also be done during execution, for example after a user action has been performed.
While this makes some test suites easier (like the one in \cref{lst:grow-and-shrink-during}), it does require more attention to the parallel nature of Scratch.
...
\marginnote{Backward compatibility is one of the downsides of using software in production.}
