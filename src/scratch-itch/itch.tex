...
When done manually, assessment of Scratch projects is time-consuming and impractical, especially in large classes, where educators often lack the time to give individual feedback to each student.
...
We also reflect on Itch's use in educational context and discuss what testing of Scratch exercises should look like.
...
\marginnote{The naming is unfortunate here.}
...
While manual test suites (also written in JavaScript) are possible, Whisker focuses on automated testing~\autocite{deinerAutomatedTestGeneration2023}.
...
\begin{listing}
    ...
    \end{scratch}
    \end{wide}
    \caption{The complete test suite for the ``Grow and shrink'' exercise in Poke.}
...
%The notion of computational thinking is not new to the education community. Studies related to computational thinking in education can be traced back
%to the 1960s when Logo programming language was first constructed (Feurzeig
%& Papert, 2011). The main purpose of the language was to help students learn
%mathematical and logical ways of thinking. It was assumed that Logo can be
%learned easily by children. The educational theory behind the utilization of Logo
%was constructionism (Papert, 1990), an interpretation of constructivism.
%Constructivism posits that individuals actively construct knowledge through
%experience. Piaget (1964) explained his ideas by stating, “To know is to
%modify, to transform the object, and to understand the process of this transformation, and as a consequence to understand the way the object is constructed” (p. 176). Papert (1993) emphasized the rich learning environments
%provided by the culture in which learners use existing tools to actively construct
%an entity. Learners then can share this entity and communicate with others
%through their own expressions. Therefore, students internalize what is outside
%and then externalize what is inside in the construction process. Although the
%Logo language was seen as a potential vehicle to transform education, it was
%not successful in fulfilling this goal. Eventually, it lost its place in the mainstream schools of the United States and United Kingdom (Agalianos, Noss, &
%Whitty, 2001).
