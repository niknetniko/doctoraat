...
We begin the chapter by giving a brief introduction and considering some related work.
...
For the test suites, we discuss their structure, although a more user-friendly format for test suites is TESTed-DSL (\cref{ch:tested-dsl}).
...
However, we believe this distinction is relevant for this chapter, as evaluating submissions (the testing framework) has a separate set of challenges compared to judge platforms (for example, displaying feedback in a constructive way).
...
The full hierarchy of the test suite thus consists of four levels, from top to bottom (\cref{fig:dodona}):
...
\begin{itemize}
    ...
    \item The return value oracle.
          This oracle compares two values (using the data serialization from \cref{subsec:data-serialization}).
...
To validate whether TESTed meets the requirements put forward in \cref{sec:tested1-programming-language-agnostic-test-frameworks}, we used it in educational practice.
