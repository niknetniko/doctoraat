...
\chapter{The \tested{} educational testing framework}\label{ch:tested1}
...
Finally, implementing support for new programming languages in TESTed is less work than implementing a complete testing framework for each individual language, reducing the software development and maintenance cost dramatically.
...
A testing framework is responsible for generating feedback, by executing and evaluating submissions based on an exercise-specific test suite.
...
However, we believe this distinction is relevant for this chapter, as evaluating submissions (the testing framework) has a separate set of challenges compared to judge platforms (e.g.\ displaying feedback in a constructive way).
This chapter focuses on testing frameworks.

In educational contexts, software testing frameworks typically use either \term{output comparison} (or \term{input/output testing}) or \term{unit testing}~\autocite{paivaAutomatedAssessmentComputer2022} to evaluate submissions.
...
With input/output testing, the judge system imposes stringent restrictions on the programming exercise: only stdin, stdout, stderr and files can be used for input and output.
...
We call a testing framework that satisfies these requirements a \emph{programming-language-agnostic testing framework}.
...
Existing formats focus mostly on the description of the task description, whereas we also focus on the specification of the test suite itself.
...
Additionally, the same data serialization is used to convert return values from the submission into the language-agnostic format for checking (as described in \vref{subsec:checking-results}).
...
TESTed partitions the test suite into \term{compilation units} (a set of contexts that are compiled together), which are in turn partitioned into execution units (a set of contexts that are executed together).
...
Next, the compilation units from the previous steps are partitioned into \term{execution units}.
\marginnote{
...
In concept, this step converts the language-agnostic test suite into actual source code, in the programming language of the test suite (an example of this is shown in \vref{sec:tested1-using-the-framework}).
...
The distinction between a judge platform and testing framework (\vref{sec:tested1-related-work}) is relevant here: Dodona is the judge platform in which different testing frameworks can be run.
...
Communication between the platform and the testing frameworks is done via a well-defined API, which are discussed in the next two sections.
As testing frameworks run student code, they must be particularly immune to bad code.
...
To achieve this, the testing frameworks are executed in \software{Docker}.
This ensures the testing frameworks are isolated from the platform and allows us to easily impose resource limits on the execution of submissions (such as memory limits, time limits, disk speed limits, network access or not).
...
When a testing framework is started by Dodona, a JSON object is provided on standard input.
...
We measured execution time of the evaluation of a correct submission for one exercise, using both TESTed and and a language-specific testing framework (\vref{fig:performance}).
