 ...
 "output": {
  "result": {
   ...
   "data": [
    ...
    \end{minted}
    \caption[]{
        Snippet of a JSON test suite for with a single test case that calls the function \texttt{remove\_all\_occurrences} with two arguments:
...
The full hierarchy of the test suite thus consists of four levels (\cref{fig:dodona}):
...
\begin{description}[noitemsep]
    ...
    \item[\texttt{any}] \marginnote{The \texttt{any} type acts roughly as the \emph{top type}, except that we do not have a formal type system, nor do we have subtypes.} Any or unknown data type.
...
\Cref{fig:flow} is a schematic overview of the evaluation process used by TESTed when evaluating a submission.
