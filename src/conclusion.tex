...
%\section{The summa summarum}\label{sec:in-summary}
%
%For textual programming languages, our contributions make it possible to ergonomically create a single programming exercise, with one test suite and one task description, and use this exercise across multiple programming languages.
%To make this possible, we created TESTed and TESTed-DSL\@.
%
%For Scratch, we make it possible to automatically assess Scratch exercises, providing students with qualitative and timely formative feedback.
%For this purpose, we created Itch, an educational testing framework for Scratch.
%Based on the feedback received from Itch, students can then use Blink, our debugger, to find the root cause of their issues.
%To end, we conclude that the Scratch execution model might benefit from some changes, but only in specific scenarios.
%In general, changing the execution model is difficult, as existing Scratch programs rely on it.
