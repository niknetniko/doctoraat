...
Within Dodona, a testing framework is called a judge (and evaluating a submission for correctness is called judging), as it was done in the Sphere Online Judge.
...
Students can then subscribe to their relevant courses (or they can be added by educators).
...
The first two questions concern themselves with textual programming languages, while the last three questions are in the context of Scratch, a visual block-based language.
...
\Cref{part:tested} is about textual programming languages and consists of two chapters (\cref{ch:tested1,ch:tested-dsl}), which answer the first and second research questions respectively.
...
\Cref{ch:itch} addresses research question three, while \cref{ch:blink} address the fourth research question.
\Cref{ch:scratch-execution-model} then answers the fifth research question.
...
It is included as \cref{ch:tested-dsl}.
