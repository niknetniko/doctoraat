...
\begin{luacode}
  function capstosc(s)
    ...
    s = string.gsub(s, "%f[%w_]SPOJ%f[^%w_]", "\\textsc{spoj}")
    s = string.gsub(s, "%f[%w_]SQL%f[^%w_]", "\\textsc{sql}")
    s = string.gsub(s, "%f[%w_]ACM-ICPC%f[^%w_]", "\\textsc{acm-icpc}")
    s = string.gsub(s, "%f[%w_]ACM%f[^%w_]", "\\textsc{acm}")
    s = string.gsub(s, "%f[%w_]ICPC%f[^%w_]", "\\textsc{icpc}")
    return s
  end
\end{luacode}
\AtBeginDocument{%
...
A comprehensive list of published work for which I am an author:
