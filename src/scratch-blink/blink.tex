...
Blink adds custom blocks to Scratch to manually program breakpoints: \scratchinline{\blockdebug{pause}}, \scratchinline{\blockdebug{pause if \emptybooldebug}}, \scratchinline{\blockdebug{wait until \emptybooldebug{} and pause}}, and \setscratch{scale=0.5}\booldebug{debugger is enabled?}.
When in debug mode, the execution of the \scratchinline{\blockdebug{pause}} block suspends the Scratch program.
...
The \scratchinline{\blockdebug{wait until \emptybooldebug{} and pause}} block waits until a condition holds and then suspends the execution.
...
\begin{figure}
    ...
    \caption[The Maze exercise.]{
        ...
        The solution is to replace this block by a \scratchinline{\blockmoreblocks{step \ovalnum{2}}} block: in which case the Player would have moved one square more to the left, meaning the Player can go up without going through a wall.
...
\begin{figure}
    ...
    \caption[The Star exercise.]{
        ...
        The second one (bottom) contains the bug: the \scratchinline{\blockmove{turn \turnright{} \ovalnum{14} degrees}} block (indicated by the red arrow) should be a \scratchinline{\blockmove{turn \turnright{} \ovalnum{144} degrees}} block.
...
In this domain, the most notable debuggers are the Microsoft MakeCode Arcade~\autocite{ballMicrosoftMakeCodeEmbedded2019a} debugger and the Blockly debugger~\autocite{savidisCompleteBlockLevelVisual2020a}.
