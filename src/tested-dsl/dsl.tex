...
```
    ...
    \caption[]{
        ...
        The sample code is denoted using the same specification of the test suite for the programming exercise in TESTed-DSL format or a reduced version thereof, see \vref{lst:recouple-example}.
    }
    \label{lst:task-description-markdown}
\end{listing}

We have included the entire specification of the sample code in the template for illustrative purposes.
Given that it is a verbatim copy of the test suite specification in \vref{lst:recouple-example}, the Jinja2 import facilities could have been used to replace the literal text with an \texttt{include} placeholder:
...
\begin{figure}
    \centerline{\includegraphics[width=\textwidth]{task-description-rendered}}
